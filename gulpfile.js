...
const uglifyjs = require('gulp-uglify');//压缩js模块
const watch = require('gulp-watch');
const sass = require('gulp-sass');
const sourcemaps = require('gulp-sourcemaps')//gulp-sourcemaps 插件 
...
gulp.task('runsass', function () {
    return gulp.src('src/scss/*.scss')
        .pipe(sourcemaps.init())// 初始化 gulp-sourcemaps 插件       
        .pipe(sass({
            outputStyle: 'compressed'
        })) //执行编译,compressed:压缩一行
        .pipe(sourcemaps.write(''))// 生成 sourcemap 文件
        .pipe(gulp.dest('dist/scss/'));// 目标文件存放路径
});
...
//     //监听的文件    监听的任务
//     watch(['src/*.html', 'src/scss/*.scss', 'src/js/*.js'], gulp.parallel('uglifyhtml', 'uglifycss', 'uglifyjs'));
